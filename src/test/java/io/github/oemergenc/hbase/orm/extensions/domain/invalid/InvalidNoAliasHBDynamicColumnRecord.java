...
@NoArgsConstructor
@AllArgsConstructor
@HBTable(name = "invalid_no_alias_column_table", families = {
        @Family(name = "staticFamily"),
        @Family(name = "invalidNoAliasDynamicFamily")
})
public class InvalidNoAliasHBDynamicColumnRecord implements HBRecord<String> {

    @HBRowKey
    @HBColumn(family = "staticFamily", column = "staticId")
    private String staticId;

    @HBDynamicColumn(family = "invalidNoAliasDynamicFamily", qualifier = @DynamicQualifier(parts = {"dynamicId"}))
    private List<DependentWithMap> noAliasDynamicFamily1;

    @HBDynamicColumn(family = "invalidNoAliasDynamicFamily", qualifier = @DynamicQualifier(parts = {"dynamicId2"}))
